...
def agent_portrayal(agent):
    if isinstance(agent, RobotLimpieza):
        return {
            "Shape": "circle", "Filled": "false", "Color": "Cyan", "Layer": 1, "r": 0.9,
            "text": f"{agent.carga}", "text_color": "black",
            "pasos": agent.movimientos, 
            "recargas": agent.recargas_completas  
        }
        ...
        if agent.sucia:
            ...
            portrayal["text"] = ""
        return portrayal
    elif isinstance(agent, EstacionCarga):  # Representación para EstacionCarga
